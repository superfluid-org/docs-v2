...
with additional features such as Payment Streams and Distributions.
...
A versioned agreement that governs the continuous flow of tokens between parties, ie [Payment Streams](/docs/protocol/money-streaming/overview.mdx).
...
Smart contract for non-fungible tokens (NFTs) that have a constant flow rate associated with them. These are NFTs minted to represent Payment Stream flows.
