...
**Flow**: A continuous payment stream from one address to another. A sender can maintain only one flow to the same receiver per token.
...
**ACL (Access Control List)**: A feature enabling varying levels of control to operators for creating, updating, or deleting payment streams on behalf of an account.

**Index**: A distribution pool created using the Instant Distribution Agreement.
...
**Sentinel**: A node monitoring the Superfluid network and closing critical or insolvent payment streams. Anyone can run a sentinel node.

**PIC (Patrician in Charge)**: The recipient of rewards for closing payment streams during the priority period when an account becomes critical.
