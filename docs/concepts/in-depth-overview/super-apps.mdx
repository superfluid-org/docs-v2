...
Super Apps represent a class of smart contracts that interact seamlessly with the Superfluid Protocol, enabling dynamic responses to Super Agreements.
...
The reactivity of Super Apps stems from **callbacks**. These are segments of code within the Super App that are triggered when a Super Agreement involving the Super App is created, updated, or deleted. Such callbacks can execute various actions, from minting NFTs to initiating new CFAs (Constant Flow Agreements).
...
1. **Incoming Flow from Account A**: Account A initiates a CFA to the Super App (100 USDCx/mo). The Super App, in turn, starts an outbound CFA of the same rate to Account Z.
2. **Multiple Flow Adjustments**: If Account B starts a new CFA to the Super App (25 USDCx/mo) and Account A adjusts its flow (to 50 USDCx/mo), the Super App responds by updating its outbound CFA accordingly.
3. **Ongoing Reactions**: The Super App continues to adapt to new CFAs or adjustments from various accounts, maintaining a reactive outbound flow.
...
- **Lending Applications**: Automate loan repayments through CFAs, eliminating the need for manual transactions.
- **Subscription Services**: Enable on-chain subscriptions paid via CFAs, incorporating features like automatic affiliate payouts.
...
[Explore Super App Examples](broken-reference)
