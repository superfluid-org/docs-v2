...
The Constant Flow Agreement (CFA) enables money streaming, a process where tokens are continuously transferred from a sender to a receiver at a defined per-second rate. This stream is perpetual and will continue until canceled by the sender or the sender's Super Token balance is depleted.
...
- **Sender**: The account initiating the CFA, specifying a receiver and flow rate.
- **Receiver**: The account on the receiving end of a CFA.
- [**CRUD Timestamp**](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete): The timestamp when a CFA is created, updated, or deleted.
...
- **Current Balance**: The sum of Static Balance and CFA Real-Time Balance.
...
The netflow for an account is calculated by netting its inbound and outbound CFA flow rates.

![Netflow Calculation](/assets/image_(63).png)

When a stream is modified, the following are updated in the Superfluid CFA contract:
...
![CFA Real-Time Balance](/assets/image_(50)_(1).png)

<Admonition type="info">
<strong>NOTE</strong>: Creating a CFA is a one-time action. The balance is dynamically calculated and does not require continuous transactions.
</Admonition>
...
Let's examine how Account A's balance changes with CFA interactions.
