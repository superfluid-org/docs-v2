...
import WrapAnimation from "@site/src/components/WrapAnimation";
import SuperTokenStreamAnimation from "@site/src/components/SuperTokenStreamAnimation";

# Super Tokens

Super Tokens extend the [ERC20 token standard](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/) with additional functionalities like [Payment Streams](./money-streaming.mdx) or [Distributions](./distributions.mdx), formerly known as Super Agreements. There are two types of Super Tokens: wrapper and custom.

:::info
Super Tokens can perform all the functions of a traditional [ERC20 token](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/), plus additional value transfer methods enabled by Superfluid, such as payment streaming.
:::
...
<SuperTokenStreamAnimation />
<div style={{ display: "flex", justifyContent: "center" }}>
  <p>*A Pure Super Token has inherent superpowers such as [Payment Streams](./money-streaming.mdx) and [Distributions](./distributions.mdx)*</p>
</div>

## Real-Time Balance

Super Tokens track an account's balance dynamically, factoring in both regular transfers and impacts from Payment Streams and Distributions.
