...
A Payment Stream is a process where tokens are continuously transferred from a sender to a receiver at a defined per-second rate. The result of this process is a "stream". A stream is perpetual and will continue until canceled by the sender/the receiver or the sender's Super Token balance is depleted.
...
Sentinels are external actors who monitor Constant Flow Agreements (Payment Streams), close streams of critical accounts, and earn buffer deposits.
