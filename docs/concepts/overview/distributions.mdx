...
While these solutions can be great for a low amount of recipients, they are not infinitely scalable and can become very expensive as the number of recipients increases. Furthermore, they do not allow for distributing payment streams
:::
...
- **Streaming Distributions**: They allow for continuous distribution of funds to receivers through [Payment Streams](./money-streaming.mdx) to a Pool.
...
**Streaming Distributions** allow for continuous distribution of funds to receivers through [Payment Streams](./money-streaming.mdx) to a Pool.
...
- **Distribution Pool**: A channel for proportional token distribution.
...
As distribution pools are also ERC20 tokens, they enable:
...
These features add composability and flexibility, expanding the potential use cases for distribution pools in web3.
