...
:::warning About the Superfluid SDK
The Superfluid SDK has been deprecated. Only critical bug fixes will be provided. We recommend using the [Superfluid Protocol](/docs/protocol/overview) directly.
Batch calls is the only feature that is not available in the Superfluid Forwarders directly and is still available in the SDK.
