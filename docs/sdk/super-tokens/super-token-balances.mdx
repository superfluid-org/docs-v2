...
- **Multi-source updates**: Super Tokens can be updated from multiple sources, from [Payment Streams](/docs/protocol/money-streaming/overview.mdx), but also [Distributions](/docs/protocol/distributions/overview.mdx).
...
The Superfluid Protocol modifies the `balanceOf` function to account for the various fund movement methods unique to Super Tokens including Payment Streams and Distributions.
...
Please keep in mind that in the example above we make the assumption that the user is only using Payment Streams and Distributions in the form of the GDA,
