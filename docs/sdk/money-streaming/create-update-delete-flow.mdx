...
The name `CFAv1Forwarder` is derived from the term "Constant Flow Agreement" (CFA) which is the underlying agreement that governs Payment Streams in Superfluid.
:::

## Interacting with the CFAv1Forwarder Contract

To interact with Payment Streams, you'll need to use the `CFAv1Forwarder` contract. Here's how to get started:
