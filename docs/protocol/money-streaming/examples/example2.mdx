...
/*...*/
```
- **Change the cooldown time limit for the game**: You can change the time limit for the game by modifying the `route.ts` file in the `app/check` directory and changing the `coolDown` variable.
