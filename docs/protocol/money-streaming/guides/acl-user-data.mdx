...
It provides a comprehensive set of tools for working with [Payment Streams](/docs/protocol/money-streaming/overview) (*Constant Flow Agreement - CFA*) and [Distribution Pools](/docs/protocol/distributions/overview) (*General Distributions Agreement - GDA*).
...
In the context of Superfluid and payment streams, Access Control Lists (ACLs) enables one address (the Flow Operator) to manage streams on behalf of another address.
...
In this document, we address Access Control for Payment Streams (CFA). Currently, there are no access control functions for [Distribution Pools](/docs/protocol/distributions/overview). However, you can build your own access control system using the SuperTokenV1Library functions.
:::
