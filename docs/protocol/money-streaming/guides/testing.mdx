...
Create a new Solidity file for your tests. Import `forge-std/Test.sol` and inherit from `Test`:
...
The `setUp` function is a **optional** special function that is executed before each test case. It is used to initialize the test environment and contract instances.
