...
- The Stream Accounting API represents payment streams in a format consumable by traditional accounting tools.
...
- Accounts for both outgoing and incoming payment streams, denoting incoming streams with positive values and outbound streams with negative values.
