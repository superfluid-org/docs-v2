...
The `flowRateAllowance` can be visualized as a tank that you fill to a certain level for the `flowOperator` account through your [`updateFlowOperatorPermissions`](sdk-core/updateflowoperatorpermissions.md) call. This allowance is then spent by the `flowOperator` as needed. Every increase in a flow rate on your behalf depletes the `flowRateAllowance`, while decreases in your flow rate or deletion of a stream do not affect it.
